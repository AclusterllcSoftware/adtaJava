...
public class ServerDBHandler {
    ...
    public JSONObject getGeneralView(int machineId) throws ParseException {
        ...
        JSONObject inductsJson = new JSONObject();
        ...
        try {
            ...
            String inductQuery = String.format("SELECT induct_id, induct_state FROM induct_states WHERE machine_id=%d ORDER BY id ASC", machineId);
            rs = stmt.executeQuery(inductQuery);

            while (rs.next())
            {
                int inductId = rs.getInt("induct_id");
                int guiId = dbCache.getGuiId(machineId, inductId, "induct");

                int inductState = rs.getInt("induct_state");

                String inductIdForHtml = "induct-" + guiId;

                inductsJson.put(inductIdForHtml, inductState);
            }
            ...
            resultJson.put("inducts", inductsJson);
